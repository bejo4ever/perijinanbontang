...
class C_trayek extends CI_Controller{
	...
	function update(){
		...
		if($ayek_updated_by == ''){
			...
			// $id_inti = $this->m_trayek_inti->__update($data_inti, $ID_TRAYEK_INTI, '','updateId');
			$id_inti = $this->m_trayek_inti->save($data_inti, $ID_TRAYEK_INTI);
			$data = array(
				'ID_TRAYEK_INTI'=>$id_inti,
			...
			$result = $this->m_trayek->__update($data, $ID_TRAYEK, FALSE, '');
		}
		echo $result;
	}
