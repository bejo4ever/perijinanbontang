...
<link rel="stylesheet" href="../../assets/jQuery/ui/themes/base/jquery.ui.all.css">	
	...
	function Tpilih(jenis,id)
		...
		{
		$.ajax({
            'url' : base_url + '/' + 'master/get_data_ho/'+id,            
            'success' : function(data){				
				...
				$("#nama_surveyor1").val(obj["nama_surveyor1"]);
				$("#jabatan_surveyor1").val(obj["instansi_jabatan1"]);
				$("#idSurveyor1").val(obj["idSurveyor1"]);

				$("#totaldatasurveyor").val(obj.totaldatasurveyor);
								
				countRow=1;
					
				if (eval(obj.totaldatasurveyor-1)>0)
					{
					var a=eval(obj.totaldatasurveyor-1);
					...
					for (var i=1;i<=a;i++)
						...
						countRow=i;						
						addDataSurveyor(0);		 									
						$("#nama_surveyor"+j).val(obj["nama_surveyor"+j]);						
						$("#jabatan_surveyor"+j).val(obj["instansi_jabatan"+j]);		
						$("#idSurveyor"+j).val(obj["idSurveyor"+j]);				
						}
					}
...
<script>
var countRow		= 1;
function addDataSurveyor(flagBaru)
{
	$(function() {		
		countRow++;
		...
		var newDiv = $(document.createElement('tr')).attr("id", 'datasurvey3' + counter1);		
    		newDiv.html('<td><div align="right">Instansi/Jabatan</div></td>'+
      					'<td><div align="center">:</div></td>'+
      					'<td>'+
						'<input type="text" name="jabatan_surveyor'+counter1+'" id="jabatan_surveyor'+counter1+'"/><input type="hidden" name="idSurveyor'+counter1+'" id="idSurveyor'+counter1+'">'+							
						'</td>');    					
			newDiv.appendTo("#datasurveyor");		
		...
		var base_url = '<?echo site_url()?>';			
		$.ajax({
            'url' : base_url + '/' + 'ho/put_surveyor/'+ho_id,            
            'success' : function(data){
				alert("Data Surveyor Berhasil Ditambah");
            },
			'error' : function(data){
               alert(data);	
            }
...
function hapusDataSurveyor(div)
   ...
   $(function() {		 
		var base_url = '<?echo site_url()?>';
		$.ajax({
            'url' : base_url + '/' + 'ho/del_surveyor/'+idSurveyor,            
            'success' : function(data){
				alert("Data Surveyor Berhasil Dihapus");
            },
			'error' : function(data){
               alert(data);	
            }
  ...
  <table width="100%"  border="0" cellspacing="1" cellpadding="1">
    <tr>
      <td height="25" colspan="3" style="padding:10px 0px 10px 0px; border-bottom:1px dashed #000000;" align="left" valign="bottom" class="font2" ><p><h1>
        <div id="wmy">Edit Data Permohonan SITU/HO
          <input type="hidden" name="id" id="id" value="<?=$id?>">
          <input type="hidden" name="ho_id" id="ho_id">
...
<br>
<input name="sbayar" type="radio" id="sbayar" onchange="SetRetribusi(this)" value="0" checked />
Belum dibayar <br>
</div>          </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><div align="center">:</div></td>
      <td><span class="text"><? echo form_input($nama_surveyor1);?>
        <input type="hidden" name="idSurveyor1" id="idSurveyor1">
      </span></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><input type="button" name="Button" value="Tambah Surveyor" onClick="addDataSurveyor(1)"></td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr>
    ...
    <tr>
      <td><div align="right">No. Reg. Izin SITU/HO</div></td>
      <td><div align="center">:</div></td>
      <td><span class="text"><? echo form_input($no_sk);?></span></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><div align="right">Berlaku sampai dengan</div></td>
      <td><div align="center">:</div></td>
      <td><input name="tglexpired" type="text" id="tglexpired" class="input-small" size="10"/></td>
    </tr>
    <tr>
      <td><div align="right">Peruntukan</div></td>
      <td><div align="center">:</div></td>
      <td><span class="text"><? echo form_input($peruntukan);?></span></td>
    </tr>
    <tr>
...
<script language="javascript">
	...
	if (confirmSubmit())
		{
		document.kirim.action="<?=site_url()."/ho/simpen_ubah"?>";
